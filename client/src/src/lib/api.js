...
export const api = {
  ...
  // Checkout APIs
  checkout: (cartItems, token) => request('/api/checkout', { method: 'POST', body: cartItems, token }),
  confirmPayment: (orderId, paymentIntentId, token) => request('/api/checkout/confirm-payment', { method: 'POST', body: { orderId, paymentIntentId }, token }),

  // Orders APIs
  getOrders: (token) => request('/api/orders', { token }),
  getAdminOrders: (token) => request('/api/orders/admin', { token }),
  updateOrderStatus: (orderId, status, token) => request(`/api/orders/admin/${orderId}/status`, { method: 'PATCH', body: { status }, token }),
}
